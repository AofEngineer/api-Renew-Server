...
const currentFile = (req, res, next) => {
  ...
  const per = req.body.person !== undefined ? req.body.person.firstname : "";
  const mem =
    req.body.members !== undefined ? req.body.members.member_name : "";
  let arc = [],
    arm = [],
    arp = [];
...
app.post("/api/upload", currentFile, (req, res, next) => {
  ...
  const dir = req.body.person;
  const reqname = req.body.firstname;
  const ext = path.extname(uploadFile.name);
  const paths = dir ? "persons" : "members";
  uploadFile.mv(`./files/${cpn}/${paths}/${dir}/${reqname}${ext}`, (err) => {
    if (err) {
